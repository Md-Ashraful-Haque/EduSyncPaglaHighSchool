...
# def generate_result_year_wise(INSTITUTE_ID, YEAR, EXAM_NAME, SHIFT):
#     print(f"\n=== Year-Wise Result has been generated. {YEAR} {[INSTITUTE_ID, YEAR, EXAM_NAME, SHIFT]}===")  
#     try:
#         exam = ExamForIMS.objects.filter(id=EXAM_NAME).first()
#         if not exam:
#             return Response(
...
#         # Apply shift filter only if not "all"
#         if SHIFT != "all":
#             classes = classes.filter(shift=SHIFT)

#         classes = classes.select_related('class_name').prefetch_related('groups')
...
def generate_result_year_wise(institute_id, year, exam_id, shift):
    print(f"\n=== Year-Wise Result has been generated. {year} {[institute_id, year, exam_id, shift]}===")  
    try:
        exam = ExamForIMS.objects.filter(id=exam_id).first()
        if not exam:
            return Response(
                {"error": "Exam not found."},
                status=status.HTTP_404_NOT_FOUND
        ...
        # Apply shift filter only if shift is not "all"
        if shift != "all":
            classes = classes.filter(shift=shift)
...
def generate_result_class_wise(institute_id, year, exam, shift, class_id):
    try:
        ...
        # Apply shift filter only if shift is not "all"
        if shift != "all":
            class_queryset = class_queryset.filter(shift=shift)

        # Get class object or raise 404
        classObj = get_object_or_404(class_queryset)
...
#         for group in classObj.groups.all():
#             debug('group', group)
#             generate_result_using_institue_to_group(institute_id, year, classObj.class_name.name, group.group_name,exam )
