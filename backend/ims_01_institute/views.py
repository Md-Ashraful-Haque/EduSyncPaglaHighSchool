...
class InstituteInfoAPIView(APIView):
    ...
    def get(self, request):
        code = request.query_params.get("instituteCode") 
        try:
            if code:
...
class MarkTypesBySubject(APIView):
    permission_classes = [AllowAny]

    def get(self, request):
        subject_for_ims_id = request.query_params.get('subject_for_ims_id')  # Clearer naming

        if not subject_for_ims_id:
            return Response(
        ...
        try:
            subject_for_ims_obj = SubjectForIMS.objects.get(id=subject_for_ims_id)
        ...
        mark_types = MarkTypeForSubject.objects.filter(subject=subject_for_ims_obj.subject_name).order_by('serial_number')
        all_mark_types = MarkTypeForSubjectSerializer(mark_types, many=True)
        # print("====================================")
        # print("all_mark_types.data: ", all_mark_types.data)
        # print("====================================")
        response_data = {
