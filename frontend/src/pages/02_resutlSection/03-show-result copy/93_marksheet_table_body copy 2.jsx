  ...
  return (
    <tbody style={{ position: "relative" }}>
      {singleMarksheetForStudent.subjects.map(
        (subject, index) =>
          (subject.total_marks > -1 || subject.pass_marks==50 || subject.is_optional === "False") && (
            <tr
              key={index}
            ...
            >
              ...
              {["WR", "MCQ", "Practical"].map((type) => {
                ...
                const match =
                  subject.marks.find((m) => m.mark_type === type) ||
                  (fallbackType &&
                    subject.marks.find((m) => m.mark_type === fallbackType));
                const mark = match ? match.marks : "-";
                const final_marks = mark == -1? "A" : mark;
                return (
                  <td
                    key={type}
                  ...
                  >
                    {/* {final_marks}  */}
                    {bySubjectVars.isBangla? (final_marks != "A" || final_marks != "-") ? showBangla(final_marks) : final_marks :  final_marks }
                  </td>
                );
              })}
              ...
              <td className="py-1 px-2 border !border-indigo-500">
                {(()=>{
                  const totalMarks = subject.total_marks < 0 ? "A" : subject.total_marks;
                  return(
                    bySubjectVars.isBangla && totalMarks != "A" ? showBangla(totalMarks) : totalMarks
                  )
                })()} 
              </td>
              <td>
