  ...
  return (
    ...
    <div className="tabulation-header">
      <div className="tabulation-student-info">
        {/* <div className="class"> 
          <strong> {isBangla ? "শ্রেণি: " : "Class: "} </strong> {firstStudent.class_name}
        </div>
        <div className="group"> 
          <strong> {isBangla ? "বিভাগ: " : "Group: "} </strong> {firstStudent.group_name}
        </div>
        <div className="section"> 
          <strong> {isBangla ? "শাখা:: " : "Section: "}</strong> {firstStudent.section_name_display}
        </div> */}
        <div className="class"> 
          <strong> {isBangla ? "শ্রেণি: " : "Class: "} </strong> { isBangla ? firstStudent.class_name : firstStudent.class_name_in_english}
        </div>
        <div className="group"> 
          <strong> {isBangla ? "বিভাগ: " : "Group: "} </strong> {  isBangla ? firstStudent.group_name_in_bangla : firstStudent.group_name }
        </div>
        <div className="section"> 
          <strong> {isBangla ? "শাখা:: " : "Section: "}</strong> { isBangla ? firstStudent.section_name_display :firstStudent.section_name }
        </div>
      </div>
      <div className="title"> 
        <span>  {isBangla ? "টেবুলেশন শীট" : "Tabulation Sheet"} </span>
      </div>
      ...
      <div className="tabulation-institute-info">
        ...
        <div className="institute-name">
          ...
          {isBangla ? examAndInstituteInfo.institute_name : examAndInstituteInfo.institute_name_eng} 
        </div>
        <div className="tabulation-exam-name">
          ...
          {isBangla ? examAndInstituteInfo.exam_name +"-"+ showBangla(examAndInstituteInfo.year) : examAndInstituteInfo.exam_name_in_english +"-"+ examAndInstituteInfo.year } 
        </div>
      </div>
    </div>
      ...
      <table  className="min-w-full bg-gray-100  noto-bangla-regular " >
        <thead>
          <tr className="text-center noto-bangla-regular">
            <td rowSpan={2} className=" ">{isBangla ? "রোল" : "Roll"}</td>
            <td rowSpan={2} className=" ">{isBangla ? "শিক্ষার্থীর নাম" : "Student Name"}</td>
            {Object.entries(subjectMarkTypes).map(([subjectName, markTypes], index) => (
              <td key={`subject-${subjectName}-${index}`} colSpan={ markTypes.length === 1 ? 1: markTypes.length + 1} className=" ">
                {/* {isBangla ? subjectName : subjectName} */}
                {subjectName}
              </td>
            ))}
            <td rowSpan={2} className=" "> {isBangla ? "মোট প্রাপ্ত নম্বর" : "Total marks obtained"} </td>
            <td rowSpan={2} className=" "> {isBangla ? "জিপিএ" : "GPA"} </td>
            <td rowSpan={2} className=" "> {isBangla ? "ফলাফল" : "Result"} </td> 
            <td rowSpan={2} className=" "> {isBangla ? "অকৃতকার্য বিষয়" : "Failed subjects"} </td>
