  ...
  return (
    <>
      <ul className={isActiveMenu ? "active-submenu" : "inactive-submenu"}>
        {/* {submenuItems
          .filter(({ menuType, inputField }) => {
            const key = permissionsMap[menuType] || permissionsMap[inputField];
            console.log("------------------------------");
            console.log(permissionsMap[menuType], permissionsMap[inputField]);
            console.log(menuType, inputField);
            console.log(vars[key]);
            console.log(vars.only_marks_input);
            console.log(key);
            console.log("------------------------------");
            return key ? vars[key] : true; // true means no restriction
          })
          .map(({ id, to, label }) => (
            <li key={id}>
              <Link
        ...
        {submenuItems.map(({ id, to, label, menuType, inputField }) => (
          menuType === "student_list" && vars.allow_students_info ? (
            <li key={id}>
              <Link
                to={to}
                className={activeSubMenuItem === id ? "active" : ""} 
